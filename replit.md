...
- **Detailed Gebäudereport = GUEST Express Checkout (no account)**: the 29 € PDF is bought via Stripe Checkout straight from the result step — no sign-up. After payment the buyer views + saves the PDF on `/report` AND receives the report link by email. The report row's `userId` is nullable; the buyer `email` is captured from the Stripe session. `/report` is a **public** route.
