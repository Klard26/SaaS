...
- `pnpm --filter @workspace/wattwechsel run dev` — enerwatt24 frontend
...
- **Shared backend**: enerwatt24 energie routes/schema stay in `api-server` + `lib/db`; both frontends call root-relative `/api/...` via the shared proxy and share the Clerk tenant (sessions carry across both apps).
- Frontend routing: Wouter with `BASE_URL` base path for the Replit proxy.

## Registration & account types

- **Customers**: after Clerk signup, go **straight to `/search`** — there is NO forced account-type chooser (the old `KontoTypWahl` / `/konto/willkommen` was removed). Booking creation needs no customer profile (it uses only the auth `customerId`). **Login/registration is uniform for every customer** — the Klard customer app no longer has any commercial account-type onboarding. Navbar "Mein Konto" → `/konto` (`MeinKonto.tsx`) is a minimal account page (Clerk identity + DSGVO `DeleteAccountSection`); the legacy `/immobilien/onboarding` route now redirects there. The whole **commercial-customer experience (Unternehmensprofil + Immobilienportfolio + verwaltete Kunden) lives in the standalone Förderschiene app** (`/foerderschiene/`), not in Klard. The Gebäudecheck also lives in Förderschiene; customer-app links point there.
...
- **Account types**: the `immobilien_kunde.typ` enum is `privat` + 7 commercial (`hausverwaltung`, `makler`, `bestandshalter`, `bautraeger`, `genossenschaft`, `handwerker`, `gewerbe`). `typ` is a `text` column, so adding values needs only spec + zod + codegen, no migration. The commercial UI (in Förderschiene `artifacts/foerderportal/src/lib/kontoTypen.ts`) hides `privat`; `privat` is kept in the enum for back-compat only.
...
- **Standalone app** (extracted out of the Klard customer app). Shares the `/api` + `/clerk` proxy and Clerk tenant like enerwatt24. Klard now only links out to it (plain cross-artifact `<a href="/foerderschiene/">`); the energie components/`printReport.ts`/`@workspace/energie-calc`/leaflet deps were removed from `@workspace/klard`.
- **Gebäudecheck = card wizard (no typing, no login)**: `Gebaeudecheck.tsx` is a multi-step click-through (Gebäudetyp → Baujahr-Bands → Wohnfläche-Ranges → Heizung → Heizungsalter → Dämmung → Fenster → Zustand). Each card maps to a representative number (bands/ranges → mid values; building type also sets representative Wohneinheiten/Geschosse via `TYP_UNITS`); `plz` stays "" (`calcEnergie` tolerates it). The final step shows the **free** Schnellcheck Energieklasse (`calcEnergie`) with no data entry.
- **Detailed Gebäudereport = GUEST Express Checkout (no account)**: the 49 € PDF is bought via Stripe Checkout straight from the result step — no sign-up. After payment the buyer views + saves the PDF on `/report` AND receives the report link by email. The report row's `userId` is nullable; the buyer `email` is captured from the Stripe session. `/report` is a **public** route.
- **Guest report access by sessionId**: `ReportPage.tsx` reconciles a single report purely by the `session_id` in the URL (the success redirect URL and the emailed link both carry it) — no `useListMyReports`, no logged-in user. Possession of the Checkout sessionId is the bearer credential.
- Report extends with funding-program matching + Einzelmaßnahmen/Komplettsanierung cost estimates, and a guided paid Energieausweis order (Bedarfs/Verbrauch, 79 €/149 €) fulfilled by a certified Aussteller (NOT app-generated; status flow). The Energieausweis order remains account-based.
- **Server-authoritative pricing** (`foerderschiene.ts`): report + Energieausweis prices come from the server, client values ignored. Report reconcile has NO ownership check (guest by sessionId). Webhook fulfillment is idempotent, raw-body + signature-verified.
- **Report-ready email** is sent once via the shared `deliverReportReadyEmail(session, baseUrl)` helper, called from BOTH the success-page reconcile AND the Stripe webhook (`checkout.session.completed` / `foerderschiene_report`), so delivery does not depend on the buyer returning to the success page. Deduped via `wasEmailSent("foerderschiene_report_ready", report.id)`.
