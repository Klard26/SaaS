...
Klard is a Doctolib-style booking marketplace for German consultants (Berater) — search, compare, and instantly book appointments with advisors across 12 professional categories. The repo also hosts **WattWechsel**, a standalone Energiewechsel cockpit that shares the same backend and Clerk tenant.

## Run & Operate

- `pnpm --filter @workspace/api-server run dev` — API server (port 8080)
- `pnpm --filter @workspace/klard run dev` — Klard frontend (port 26057)
- `pnpm --filter @workspace/wattwechsel run dev` — WattWechsel frontend (port 18607)
...
- Use these instead of re-declaring local STATUS maps, tier pills, ad-hoc number-circle steppers, or empty states. Stepper labels: customer flow = Leistung·Termin·Bestätigung; provider onboarding = Profil·Leistungen·Verfügbarkeit; commercial customer = Kundenprofil·Gebäudecheck.
