...
- **Two separate registration tracks**:
  - **Customers** ("Einloggen" / "Kostenlos registrieren" in the navbar): after Clerk signup, `SignUpPage` redirects to `/konto/willkommen` (`KontoTypWahl.tsx`) where the user picks one of 7 account types. `privat` saves a minimal profile inline (firma = user name) → `/search`; the 6 commercial types route to `/immobilien/onboarding?typ=<value>`.
  - **Berater (providers)**: dedicated public landing `/berater-werden` (`BeraterWerden.tsx`). CTA routes signed-in users to `/provider/onboarding`, signed-out to `/sign-up?intent=berater`. `SignUpPage` reads `?intent=berater` and force-redirects to `/provider/onboarding` instead of the customer chooser.
...
- **Triggers** (template → event):
  ...
  - `booking_reminder_24h` via in-process scheduler (`reminderScheduler.ts`): polls every 15 min for bookings 24–25h ahead with status `pending`/`confirmed` and `reminderSentAt IS NULL`; marks `reminderSentAt` after sending.
- **Unwired templates** (provided but no trigger yet — would need new infrastructure): `booking_reminder_1h` (scheduler only does 24h, single `reminderSentAt` flag), `payment_failed` (booking checkout is synchronous; no failed-payment event captured), `profile_activated` (no distinct "profile published" event — `welcome_provider` already covers provider creation).
