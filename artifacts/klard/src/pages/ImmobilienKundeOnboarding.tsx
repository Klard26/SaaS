...
const schema = z.object({
  typ: z.enum(["hausverwaltung", "bestandshalter"]),
...
export default function ImmobilienKundeOnboarding() {
  ...
  useEffect(() => {
    if (existing) {
      form.reset({
        typ: existing.typ as "hausverwaltung" | "bestandshalter",
    ...
    try {
      await upsert.mutateAsync({
        data: {
          ...
          anzahlGebaeude:
            values.anzahlGebaeude === undefined || Number.isNaN(values.anzahlGebaeude)
              ? null
              : values.anzahlGebaeude,
          wohneinheitenGesamt:
            values.wohneinheitenGesamt === undefined || Number.isNaN(values.wohneinheitenGesamt)
              ? null
              : values.wohneinheitenGesamt,
        },
      });
      qc.invalidateQueries({ queryKey: getGetMyImmobilienKundeQueryKey() });
      toast({ title: "Gespeichert", description: "Ihr Immobilien-Profil wurde aktualisiert." });
      setLocation("/gebaeudecheck");
    } catch (err) {
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 flex-1 w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground">Als Hausverwalter / Bestandshalter registrieren</h1>
          <p className="text-muted-foreground mt-2">
            Hinterlegen Sie Ihr Immobilien-Profil für den Gebäudecheck. Portfolio-Angaben sind optional.
          </p>
        </div>

        <Card>
          <CardHeader className="pb-4">
            <CardTitle className="text-base">Ihr Immobilien-Profil</CardTitle>
                  ...
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="bestandshalter">Bestandshalter</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
